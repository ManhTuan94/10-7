...
//	[layer_1 setFrame:CGRectMake(0, 0, layer_1.frame.size.width * recognizer.scale, layer_1.frame.size.height * recognizer.scale)];
//    layer_1.center = center;

//	CGPoint center2 = layer_2.center;
//	[layer_2 setFrame:CGRectMake(0, 0, layer_2.frame.size.width * recognizer.scale, layer_2.frame.size.height * recognizer.scale)];
//    layer_2.center = center2;
//
//    
//    if (sliderRotate.value == 0) {
//        [layer_2 setFrame:layer_1.frame];
//        layer_2.center = CGPointMake(layer_1.frame.size.width/2, layer_1.frame.size.height/2);
//        width = layer_1.frame.size.width;
//        height = layer_1.frame.size.height;
//    }else{
...
////////////
    CGPoint center = layer_1.center;
    layer_2.backgroundColor = [UIColor redColor];
    CGRect rect = layer_1.frame;
    ...
    if (rotation == 0){
        ...
        layer_1.bounds = CGRectMake(0, 0, layer_1.bounds.size.width * recognizer.scale, layer_1.bounds.size.height * recognizer.scale);
//        layer_1.center = center;
        rotated = CGRectApplyAffineTransform([layer_1 bounds], [layer_1 transform]);
        [layer_2 setBounds:CGRectMake(0, 0, rotated.size.width, rotated.size.height)];
        layer_2.center = CGPointMake(layer_1.frame.size.width/2, layer_1.frame.size.height/2);
        NSLog(@"%@ layer2 center",NSStringFromCGPoint(layer_2.center));

    }
    
    [layer_pixellate setBounds:CGRectMake(-rotated.origin.x, -rotated.origin.y, layer_pixellate.frame.size.width, layer_pixellate.frame.size.height)];
    
    NSLog(@"%@ layer1 frame before",NSStringFromCGRect(rect));
    NSLog(@"%@ layer1 frame after",NSStringFromCGRect(layer_1.frame));
    NSLog(@"%@ layer2 bounds",NSStringFromCGRect(layer_2.bounds));
    NSLog(@"%@ layer2 frame",NSStringFromCGRect(layer_2.frame));


//////////////
//    layer_pixellate.center = CGPointMake(layer_2.bounds.size.width/2, layer_2.bounds.size.height/2);
//    layer_1.transform = CGAffineTransformScale(layer_1.transform, recognizer.scale, recognizer.scale);
//    CGRect layer2FrameAfter = [self.layer_1 convertRect:self.layer_2.frame fromView:self.originalImageView];
////    layer_pixellate.transform = CGAffineTransformScale(layer_pixellate.transform, 1.0/recognizer.scale, 1.0/recognizer.scale);
//    layer_2.backgroundColor = [UIColor redColor];
//    layer_pixellate.frame = CGRectMake(-layer_1.frame.origin.x, layer_1.frame.origin.y, layer_pixellate.frame.size.width, layer_pixellate.frame.size.height);
    recognizer.scale = 1;
//    layer_pixellate.frame = CGRectMake( layer2FrameAfter.origin.x, layer2FrameAfter.origin.y, layer_pixellate.frame.size.width, layer_pixellate.frame.size.height);
//    NSLog(@"layer2:%@",NSStringFromCGRect(layer2FrameAfter));
}
...
- (IBAction)Rectange:(id)sender {
    ...
    RectangleShape* shape = [[RectangleShape alloc] init];
...
- (IBAction)ellipse:(id)sender {
    ...
    layer_1.transform = CGAffineTransformMakeRotation(0);
    layer_2.transform = CGAffineTransformMakeRotation(0);
    EllipseShape* shape = [[EllipseShape alloc] init];
...
-(void)save{
    ...
    if(num == 2) {
//        CGPoint center = layer_1.center;
        UIImageView* subSelectedView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, width, height)];
        ...
        subSelectedView.layer.borderWidth = 2;
        subSelectedView.layer.borderColor = [UIColor blueColor].CGColor;
